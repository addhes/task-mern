...
const TaskListTable = ({ tableData }) => {
    ...
    return (
                ...
                <tbody>
                    {tableData.map((task) => (
                        <tr key={task._id} className=' border-t border-gray-100 '>
                            <td className=' py-3 mx-4 text-xs text-gray-600 line-clamp-1 overflow-hidden'>{task.title}</td>
                            ...
                            <td className=' text-gray-600 px-3 py-3 text-[13px] text-nowrap hidden md:table-cell'>
        ...
        </div> // isi elemen akan ditambahkan nanti
    );
}
